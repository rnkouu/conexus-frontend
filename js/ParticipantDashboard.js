...
(function(){
  ...
  // UPDATED: Now receives rooms and dorms arrays to look up the assigned location
  function RegistrationDetailsModal({ reg, event, rooms = [], dorms = [], onClose }) {
    ...
    let assignedRoomName = "Pending Assignment";
    let assignedDormName = "";
    
    if (reg.roomId) {
        const foundRoom = rooms.find(r => String(r.id) === String(reg.roomId));
        if (foundRoom) {
            assignedRoomName = `Room ${foundRoom.name}`;
            const foundDorm = dorms.find(d => String(d.id) === String(foundRoom.dormId));
            if (foundDorm) {
  ...
  function ParticipantDashboard({
    ...
    onSubmitPaper,
    onUpdateUser,
    // NEW: We expect rooms and dorms to be passed in from App.js
    rooms = [],
    dorms = []
  }) {
    ...
    return (
      <section className="relative px-4 py-10 max-w-7xl mx-auto animate-fade-in-up">
        ...
        {/* NEW UNIFIED HEADER WITH ANIMATED MENU */}
